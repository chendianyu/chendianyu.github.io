...
当然不同类型 T 细胞之间，部分通路的调控也存在差异。对于 CD8+ T 细胞（下图），发现分支8为高度增殖性（有丝分裂，G2M 检查点，E2F 靶标）；分支4和8具有高度同种异体移植物排斥活性，可能是对肿瘤细胞编码的新生抗原表现出更强的反应。另外，这两个分支均具有更强的 IFN-γ 和 IFN-α 反应性，更快的转录速率，Myc 活性和高颗粒酶表达，均为 T 细胞活化特征。  
