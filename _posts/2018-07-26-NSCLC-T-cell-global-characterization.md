---
title: scRNA-seq 揭示非小细胞肺癌中 T 细胞全景
description: 
categories:
 - Lung Cancer
tags:
...
# Highlights  
1. 高深度单细胞测序从 NSCLC 中共找到7个 CD8 分支和9个 CD4 分支，不同分支在各组织间的分布存在差异  
2. 不同分支 TCR 克隆扩展模式不同，其中 CD8-C3-CX3CR1 和 CD4-C3-GNLY 两个效应分支表现出高迁移性的特征  
3. 发育轨迹分析表明 NSCLC 中 CD8+ T 细胞的状态是由固有的 T 细胞发育程序和肿瘤诱导的 T 细胞耗竭两个不同过程共同决定的，CD4+ T 细胞也是类似的  
4. Trm 中存在抑制性，其中 CD8-C4-GZMK 和 CD8-C5-ZNF683 可能为耗竭前状态，在 LUAD 中与更佳的预后表现相关  
5. Treg 中的 CD4-C9-CTLA4 分支内 TNFRSF9 存在双峰表达特性，其中 TNFRSF9+ 细胞经过抗原刺激的，表现免疫抑制功能，对应更差的预后  
6. T 细胞组成可能可用于 NSCLC 病人的分类，不同的 T 细胞群体可被不同靶向的免疫疗法针对
  
# Main
NSCLC 是肿瘤相关致死率中的主要原因，占了所有肺癌病例的约85%。NSCLC 肿瘤通常具有极多的基因组变异，高免疫负荷，因此普遍对免疫检查点抑制疗法反应更好。为了揭示 NSCLC 中肿瘤浸润性 T 细胞的复杂性，作者从14位未接受治疗的病人的肿瘤，相邻正常组织以及外周血中分离出 T 细胞。这14位病人中有11位为腺癌（adenocarcinomas），3位为鳞细胞癌（squamous cell carcinomas）。确认 NSCLC 中存在 T 细胞浸润后，作者利用流式细胞术，基于 CD3/CD4/CD8/CD25 细胞表面标记，将各种 T 细胞亚型进行分选，然后进行深度单细胞转录组测序。最终，作者共计获得12346个细胞，平均测序深度为每个细胞104万对单一匹配的 read，足够检测低表达的细胞因子和转录因子。  
...
为了更好的理解发育轨迹，作者根据以前研究中确定的特征基因定义了初始性分值和细胞毒性分值，以及基于肿瘤浸润性耗竭 CD8<sup>+</sup> T 细胞中高表达的90个基因的平均表达量定义了 T 细胞耗竭分值。这90个基因与从肝细胞癌，黑色素瘤以及感染了淋巴细胞性脉络丛脑膜炎病毒的小鼠模型中鉴定出来的耗竭相关基因存在明显的重叠。基于人类肿瘤研究得到的23个基因中有14个在小鼠研究中并未发现，可能代表人类特有的耗竭标志基因。对比 Monocle 中成分与这些功能性分值之间的关系，发现成分1与 T 细胞耗竭高度相关，成分2与细胞毒性正相关，与初始性负相关（下图）。这些结果表明 NSCLC 中 CD8<sup>+</sup> T 细胞的状态是由固有的 T 细胞发育程序和肿瘤诱导的 T 细胞耗竭两个不同过程共同决定的。  
...
CD8-C4-GZMK 和 CD8-C5-ZNF683 位于发育轨迹中心，耗竭分值相较 CD8-C6-LAYN 更低，表明可能是处于“耗竭前”状态。这些细胞在肝癌中（HCC）也存在，不过 CD8-C5-ZNF683 细胞在 NSCLC 中更加富集（下图），意味着它们可能在 NSCLC 免疫中发挥作用。  
...
两个 Treg 分支中，CD4-C8-FOXP3 主要存在于血液中, 而 CD4-C9-CTLA4 则主要存在于肿瘤中。Treg 的克隆扩增是最具分支特异性的，绝大部分只在肿瘤中存在，表明**抑制性肿瘤驻留 Tregs 的局部性克隆性扩展**。CD4-C9-CTLA4 相较 CD4-C8-FOXP3 具有更多的克隆扩增的细胞，且有404个基因表达量至少高2倍。这些基因与之前针对黑色素瘤，乳腺癌，结直肠癌和肺癌的研究中所发现的肿瘤驻留 Tregs 中的基因存在高度重叠性，表明血液和肿瘤中 Treg 之间的差异。  
