...
## 深度 
* `AD (DepthPerAlleleBySample)` 输出的是每个样本中每个等位基因的未经过滤的测序深度，避免因为过滤可能引入的偏差，即便这些 reads 质量很差  
* `DP (Coverage)` 输出的则是每个样本经过过滤的测序深度（在 FORMAT 中时）和所有样本未经过滤的测序深度（在 INFO 中时）。所以在样本层面其与 AD 之间可能存在不一致，需要注意  
