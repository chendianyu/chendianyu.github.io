...
# TCR 身份揭示 HCC 中 CD8 T 细胞和 Tregs 的富集  
TCR 经常被用作确定 T 细胞祖源的唯一标识符，因此作者用 TraCeR 方法将从5位 HBV 阳性的 HCC 患者细胞获取的单细胞转录组数据进行拼接，得到全长 TCR α 和 β 序列，其中94%的 T 细胞至少能得到一条能生成产物的 TCR α-β 链用于后续分析。虽然绝大部分细胞表达唯一的 TCR α 和 β 等位基因，但仍有一部分 T 细胞的 α 和 β 序列会有重复。  
为了充分明确 T 细胞克隆性，作者定义至少有一对相同的 α-β 链的 T 细胞克隆自同一祖先，扩大的克隆则被定义为在给定细胞群体中，至少有三个细胞共享 α 和 β 序列
