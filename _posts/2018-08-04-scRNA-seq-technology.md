...
首先来看单细胞的分离：  
* 最早是显微操作，一个个地分离；对于组织中的细胞，可以通过激光捕获显微切割（laser capture microdissection，LCM）技术进行分离。显微操作方法优点是能够捕获所有细胞，缺点是耗时和劳动密集型  
* 为了增加通量，组织中的细胞先进行分离，悬浮于培养液中。要注意的是有些情况下细胞难以分离，另外就是酶试剂等可能对细胞产生影响，改变表达谱。细胞的分选可以通过流式细胞仪进行，从而快速将单个细胞分选至微孔内，此外还能够通过荧光标记富集特定细胞群。使用微孔板方法的主要缺点是它不能像微流体方法一样，将反应缩减至纳升级体积，从而导致每个细胞平均的试剂消费较高  
* Fluifigm C1 微流体平台，能够通过微流循环捕获细胞。每个芯片最多能够捕获96个细胞，之后在纳升体积中进行 RT 和预扩增。该方法对细胞大小存在要求，然后对具有粘性或者非球形的细胞的捕获效率较低，另外就是每次捕获需要至少1000个细胞输入，因此不适合原材料有限的情况。每个 IFC 芯片的价格较贵，但减少了试剂的消耗  
* 最后就是基于微滴的微流方法，能够捕获成千上万个细胞，将反应缩减至纳升甚至皮升级别，减少试剂消耗  
